...
флаг `--debug` при запуске интерпретатора. При этом будет включен интерактивный режим: интерпретатор будет ждать от вас
команд через stdin.

Все этапы до этапа исполнения байт-кода работают без изменений. Однако перед исполнением байт-кода интерпретатор
останавливается. Его действиями можно управлять с помощью следующих команд:
